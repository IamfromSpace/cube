...
mod tests {
    ...
    fn permutation_is_associative() {
        ...
        for pi_0 in all::<TwoLinesIndex>() {
            let p_0: TwoLines = pi_0.into();
            for pi_1 in all::<TwoLinesIndex>() {
                let p_1: TwoLines = pi_1.into();
                for pi_2 in all::<TwoLinesIndex>() {
                    let p_2: TwoLines = pi_2.into();
    ...
    fn into_and_full_symmetry_commute_on_turns() {
        for s in all::<FullSymmetry>() {
            for t in all::<Turns>() {
                assert_eq!(Into::<TwoLines>::into(t).get_equivalent(&s), t.get_equivalent(&s).into())
            }
        }
    }
