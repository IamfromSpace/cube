...
impl From<G1aWideTurn> for CoordWingEdgesLockedUDOdds {
...
mod tests {
    ...
    quickcheck! {
        fn perm_and_g1a_turns_and_sym_invert_round_trips(pi: CoordWingEdgesLockedUDOddsIndex, t: G1aWideTurn, s: U2F2Symmetry) -> bool {
            let p: CoordWingEdgesLockedUDOdds = pi.into();
            p == p.invert().invert()
    ...
    quickcheck! {
        fn perm_and_g1a_turn_full_symmetries_are_equivalent_through_uf2(pi: CoordWingEdgesLockedUDOddsIndex, t: G1aWideTurn, s: UF2Symmetry) -> bool {
            let p: CoordWingEdgesLockedUDOdds = pi.into();
    ...
    quickcheck! {
        fn perm_and_g1a_turn_full_symmetries_are_equivalent_through_u2f2(pi: CoordWingEdgesLockedUDOddsIndex, t: G1aWideTurn, s: U2F2Symmetry) -> bool {
            let p: CoordWingEdgesLockedUDOdds = pi.into();
            let after_permute = p.permute(t.into()).get_equivalent(&s);
            let before_permute = p.get_equivalent(&s).permute(t.get_equivalent(&s).into());
            after_permute == before_permute
    ...
    quickcheck! {
        fn perm_and_g1a_turn_full_symmetries_are_equivalent_through_u2(pi: CoordWingEdgesLockedUDOddsIndex, t: G1aWideTurn, s: U2Symmetry) -> bool {
            let p: CoordWingEdgesLockedUDOdds = pi.into();
            let after_permute = p.permute(t.into()).get_equivalent(&s);
            let before_permute = p.get_equivalent(&s).permute(t.get_equivalent(&s).into());
            after_permute == before_permute
    ...
    quickcheck! {
        fn into_and_uf2_symmetry_commute_on_g1a_wide_turn(s: UF2Symmetry, t: G1aWideTurn) -> bool {
            Into::<CoordWingEdgesLockedUDOdds>::into(t).get_equivalent(&s) == t.get_equivalent(&s).into()
        }
    }
